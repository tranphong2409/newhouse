...
function count_number_new($id){
	
	global $d;
	
		$d->query("select count(*) as num from #_product where id_thanhvien='".$id."'");
...
function giaca($str){ 
  ...
  return $str;
    ...
    // {
	    ...
	    // {
	       	// $ty = round($so / 1000000000);
	       	// $gia = $ty." tỷ";
	       	// $trieu = round(($so%1000000000) / 1000000);
	       	// if($trieu>0){
		       	// $gia .= " ".$trieu." triệu";
		    // }
	    // }
...
function get_tinhthanh($table="tinh",$id,$more=""){
	$id = intval($id);
	global $d;
	if($id==0){
		$d->query("select id,ten,tenkhongdau from #_$table where hienthi=1 $more order by stt,id asc");
		return $d->result_array();
	}else{
		$d->query("select id,ten,tenkhongdau from #_$table where hienthi=1 $more and id=".$id);
		return $d->fetch_array();
	}
}
...
function upload_image_at($file, $extension, $folder, $newname=''){
...
function pagination($query,$per_page=10,$page=1,$url='?'){   
    ...
    $prevlabel = _trangsau;
    $nextlabel = _trangtiep;
    $lastlabel = _trangcuoi;
    ...
    $pagination = "";
    if($lastpage > 1){   
        $pagination .= "<div class='paging'><ul class='pagination'>";
...
function get_info_tv1($email){
	global $d;
	$d->query("select * from #_thanhvien where id='".$email."' ");
	return $d->fetch_array();
}
