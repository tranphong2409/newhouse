...
switch($act){
	case "man_list":
		get_lists();
		$template = "albumphongngu/list/items";
		break;
	case "add_list":		
		$template = "albumphongngu/list/item_add";
		break;
	case "edit_list":		
		get_list();
		$template = "albumphongngu/list/item_add";
	...
	case "man":
		get_mans();
		$template = "albumphongngu/man/items";
		break;
	case "add":		
		$template = "albumphongngu/man/item_add";
		break;
	case "edit":		
		get_man();
		$template = "albumphongngu/man/item_add";
...
function get_mans(){
	...
	$url = "index.php?com=albumphongngu&act=man&type=".$_GET['type']."".$link_add;
...
function get_man(){
	...
	$sql = "select * from #_albumphongngu where id='".$id."'";
	$d->query($sql);
	if($d->num_rows()==0) transfer("Dữ liệu không có thực", $_SESSION['links_re']);
	$item = $d->fetch_array();	

	$sql = "select * from #_albumphongngu_photo where id_album='".$id."' and type='".$_GET['type']."' order by stt,id desc ";
	$d->query($sql);
...
function delete_man(){
	...
	if(isset($_GET['id'])){
		$id =  themdau($_GET['id']);
		$d->reset();
		$sql = "select id,photo,thumb from #_albumphongngu where id='".$id."'";
		$d->query($sql);
		if($d->num_rows()>0){
			...
			$sql = "delete from #_albumphongngu where id='".$id."'";
			$d->query($sql);
		}
		if($d->query($sql))
			redirect($_SESSION['links_re']);
...
function get_lists(){
    ...
    $url = "index.php?com=albumphongngu&act=man_list&type=".$_GET['type']."".$link_add;
	$paging = pagination($where,$per_page,$page,$url);
}
...
function get_list(){
	...
	$sql = "select * from #_albumphongngu_list where id='".$id."'";
	$d->query($sql);
	if($d->num_rows()==0) transfer("Dữ liệu không có thực", $_SESSION['links_re']);
	$item = $d->fetch_array();
}
...
function delete_list(){
	...
	if(isset($_GET['id'])){
		$id =  themdau($_GET['id']);
		$d->reset();
		$sql = "select id,photo,thumb from #_albumphongngu_list where id='".$id."'";
		$d->query($sql);
		if($d->num_rows()>0){
			while($row = $d->fetch_array()){
				delete_file(_upload_album.$row['photo']);
			...
			$sql = "delete from #_albumphongngu_list where id='".$id."'";
			$d->query($sql);
		}
		if($d->query($sql))
			redirect($_SESSION['links_re']);
