<script type="text/javascript">
  ...
  function select_quanhuyen()
  {
    var a=document.getElementById("id_quanhuyen");
    window.location ="index.php?com=product&act=man&type=<?=$_GET['type']?>&id_quanhuyen="+a.value; 
    return true;
  }
  function select_loaidat()
  {
    var a=document.getElementById("id_loaidat");
    window.location ="index.php?com=product&act=man&type=<?=$_GET['type']?>&id_loaidat="+a.value; 
    return true;
  }
...
<?php
  ...
  function get_main_quanhuyen()
  {
    $sql="select * from table_baiviet_cat where type='tinhthanh' order by stt asc";
    $stmt=mysql_query($sql);
    $str='
      <select id="id_quanhuyen" name="id_quanhuyen" onchange="select_quanhuyen()" class="main_select">
      <option value="">Chọn quận/huyện</option>';
    while ($row=@mysql_fetch_array($stmt)) 
    {
      if($row["id"]==(int)@$_REQUEST["id_quanhuyen"])
        $selected="selected";
      else 
        $selected="";
  ...
  function get_main_loaidat()
  {
    $sql="select * from table_baiviet where type='loaidat' order by stt asc";
    $stmt=mysql_query($sql);
    $str='
      <select id="id_loaidat" name="id_loaidat" onchange="select_loaidat()" class="main_select">
      <option value="">Chọn loại đất</option>';
    while ($row=@mysql_fetch_array($stmt)) 
    {
      if($row["id"]==(int)@$_REQUEST["id_loaidat"])
        $selected="selected";
      else 
        $selected="";
      $str.='<option value='.$row["id"].' '.$selected.'>'.$row["ten_vi"].'</option>';      
  ...
  <table cellpadding="0" cellspacing="0" width="100%" class="sTable withCheck mTable" id="checkAll">
    <thead>
      <tr>
        ...
        <td class="tb_data_small"><?=get_main_quanhuyen()?></td> 
        <td class="tb_data_small"><?=get_main_loaidat()?></td> 
  
        <td class="sortCol"><div>Tên <?=$title_main?><span></span></div></td>
        <?php if($config_noibat == "true"){ ?>
        <td class="tb_data_small">Nổi bật</td>
        <?php } ?>
        <?php /*<td class="tb_data_small">SP thông dụng</td>
    ...
    <tbody>
       ...
       <td>
        ...
        <td align="center">
          <?php
            $d->reset();
            $sql = "select ten_vi from table_baiviet_cat where id='".$items[$i]['id_quanhuyen']."'";
         ...
         <td align="center">
          <?php
            $d->reset();
            $sql = "select ten_vi from table_baiviet where id='".$items[$i]['id_loaidat']."'";
            $result=mysql_query($sql);
            $name_danhmuc =mysql_fetch_array($result);
            echo @$name_danhmuc['ten_vi'];
          ?>  
