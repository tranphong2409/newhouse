...
<form name="supplier" id="validate" class="form" action="index.php?com=baiviet&act=save<?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>" method="post" enctype="multipart/form-data">
    ...
    <?php }?>
        ...
        <?php if($_GET['type']=='loaidat'){ ?>
        <div class="formRow">
			<label>Loại bất động sản</label>
			<div class="formRight">
                <select id="id_bds" name="id_bds[]" class="main_select" multiple="multiple" >
                <?php $row_bds = explode(',', $item['id_bds']); ?>
                	<option value="1" <?php if(in_array('1',$row_bds)){ ?>selected="selected"<?php } ?>>- Nhà đất bán</option>
                	<option value="2" <?php if(in_array('2',$row_bds)){ ?>selected="selected"<?php } ?>>- Dự án</option>
                	<option value="3" <?php if(in_array('3',$row_bds)){ ?>selected="selected"<?php } ?>>- Cho thuê</option>
                </select>
			</div>
			<div class="clear"></div>
		</div>
		...
		<div class="formRow lang_hidden lang_vi active">
			<label><?php if($_GET['type']=='tinvideo'){ ?>Link (nếu có)<?php }else{ ?>Mô tả <?php } ?></label>
		...
		<?php if($_GET['type']=='hethong'){ ?>
		<div class="formRow">
			<label>Tọa độ</label>
			<div class="formRight">
                <input type="text" name="toado" title="Nhập tọa độ" id="toado" class="tipS validate[required]" value="<?=@$item['toado']?>" />
			</div>
			<div class="clear"></div>
		</div>
		<?php } ?>
